...
from custom_components.spa_care.const import CONF_NAME, CONF_VOLUME_L, DOMAIN
...
async def test_user_flow_creates_entry(hass: HomeAssistant):
    ...
    with patch(
        ...
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            user_input={CONF_NAME: "Hot Tub", CONF_VOLUME_L: 1500.0},
        )

    assert result["type"] == data_entry_flow.FlowResultType.CREATE_ENTRY
    assert result["title"] == "Hot Tub"
    assert result["data"][CONF_NAME] == "Hot Tub"
...
async def test_user_flow_rejects_zero_volume(hass: HomeAssistant):
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        user_input={CONF_NAME: "Spa", CONF_VOLUME_L: 0.0},
    )
    assert result["type"] == data_entry_flow.FlowResultType.FORM
...
async def test_user_flow_rejects_empty_name(hass: HomeAssistant):
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        user_input={CONF_NAME: "", CONF_VOLUME_L: 1500.0},
    )
    assert result["type"] == data_entry_flow.FlowResultType.FORM
    assert result["errors"] is not None
